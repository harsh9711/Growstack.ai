...
const NavLink = ({ item, onToggleSubmenu, onCloseMobileMenu }) => {
                                ...
                                return (
                                    ...
                                    >
                                        <span className='flex gap-2 items-center overflow-hidden'>
                                            <img src={nav.img} alt={nav.title} className='w-6 h-6 flex-shrink-0' />
                                            <span className='truncate'>{nav.title}</span>
                                        </span>
                                        <img src='/images_growstack/header/arrow.svg' alt='arrow' className='w-2 h-2 flex-shrink-0' />
                                    </Link>
                                );
                            }
                        })}
    ...
    return (
        <div className='navLink'>
            ...
            {hasSubmenu && (
                <div className={item.title === "Solutions" ? "submenusolutions" : "submenu"} style={{ display: item.isOpen ? "block" : "none" }}>
