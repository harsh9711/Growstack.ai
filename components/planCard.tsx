...
const PlanCard = ({
  ...
  const { currentPlan, isAuthenticated } = useSelector(
  ...
  const isDowngrade =
    (isUpgradePlan &&
      currentPlan &&
      ((selectedTabIndex === 0 &&
        parseFloat(plan.monthlyPrice!) < currentPlan.plan_amount) ||
        (selectedTabIndex === 1 &&
          parseFloat(plan.yearlyPrice!) < currentPlan.plan_amount))) ||
    false;
    ...
    <>
      <div className="items-center justify-center mx-auto w-full bg-[#F5F5F5] rounded-xl flex flex-col py-6  hover:scale-[102%] border-transparent hover:shadow-lg hover:bg-white transition-all duration-500 shadow-sm hover:border-[4px]">
        <div className="text-center w-full flex flex-col">
          ...
          <p className="px-8  opacity-60 w-full h-full lg:h-[120px] xl:h-[110px]  mx-auto">
            {plan.description}
          </p>
          <div className="border-[#B8B8B8] px-10 border w-full mt-6 mb-6"></div>
        </div>
        <div
          className={`flex flex-col gap-y-2 px-4 ${marginBottom} w-full overflow-y-auto sm:h-[320px] sm:overflow-y-scroll`}
        >
      ...
      >
        <div className="flex flex-col items-center justify-center px-6 pt-4 pb-8 gap-6 space-x-6">
          ...
          <p className="text-center text-gray-600 md:text-base px-4">
            Enjoy our services with enhanced features and benefits. We're
            excited to have you onboard with the upgraded plan!
          </p>
