import React, { useEffect } from "react";
import AOS from "aos";
import "aos/dist/aos.css";
import Link from "next/link";
import "./Cta.scss";
function Cta() {
    useEffect(() => {
        AOS.init();
    }, []);
    return (
        <React.Fragment>
            <div className='cta'>
                <div className='container'>
                    <div className='flex flex-col sm:flex-row'>
                        <div className='w-full sm:w-1/2 ' data-aos='fade-right' data-aos-easing='ease-in-sine' data-aos-duration='1000'>
                            <span className='user'>Take Control</span>
                            <h3>
                                On-The-Go with our mobile app <span>integrated CRM platform built for success</span>
                            </h3>
                            <p>
                                Stay productive anywhere with real-time control and insights, effortlessly managing your business with our intuitive
                                mobile app
                            </p>
                            <Link href='/auth/register' className='sheen flex flex-row getStart'>
                                Get Started{" "}
                                <svg width='21' height='16' viewBox='0 0 21 16' fill='none' xmlns='http://www.w3.org/2000/svg'>
                                    <path
                                        d='M20.5061 8.70711C20.8966 8.31658 20.8966 7.68342 20.5061 7.29289L14.1421 0.928932C13.7516 0.538408 13.1184 0.538408 12.7279 0.928932C12.3374 1.31946 12.3374 1.95262 12.7279 2.34315L18.3848 8L12.7279 13.6569C12.3374 14.0474 12.3374 14.6805 12.7279 15.0711C13.1184 15.4616 13.7516 15.4616 14.1421 15.0711L20.5061 8.70711ZM0 9L19.799 9V7L0 7L0 9Z'
                                        fill='#fff'
                                    />
                                </svg>
                            </Link>
                            {/* <div className='store'>
                                <Link href='/auth/register'>
                                    <img src='/images_growstack/banner/playStore.svg' alt='banner' />
                                </Link>
                                <Link href='/auth/register'>
                                    <img src='/images_growstack/banner/apple.svg' alt='banner' />
                                </Link>
                            </div> */}
                        </div>
                        <div className='ctaImg'>
                            <img
                                data-aos='fade-left'
                                data-aos-easing='ease-in-sine'
                                data-aos-duration='1000'
                                src='/images_growstack/home/cta.png'
                                alt='cta'
                            />
                        </div>
                    </div>
                </div>
            </div>
        </React.Fragment>
    );
}

export default Cta;
