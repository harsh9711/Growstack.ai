...
export default function FeatureItem(props: Props) {
  ...
  return (
    <div
      ...
      className={cn(
        "text-center  inline-flex flex-col items-center justify-center max-w-28 xl:max-w-48 px-3",
        className
      )}
    >
      {props?.icon ? (
        <div className=" w-8 h-8 2xl:w-[85px] 2xl:h-[89px] mx-auto">
          <Image
            ...
            width={85}
            height={86}
            alt={props?.icon.alt ? props?.icon.alt : title}
      ...
      <span className="inline-block -mt-2 text-[12px] 2xl:text-[16px] ">
        {title}
      </span>
    </div>
  );
