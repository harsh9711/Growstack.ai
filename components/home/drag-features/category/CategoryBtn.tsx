...
export default function CategoryBtn(props: CategoryProps) {
  ...
  return (
    ...
    >
      <span
        className={cn(
          "relative inline-flex items-center justify-center size-3 2xl:size-4 rounded-full border border-current before:inline-block before:size-3 before:rounded-full before:bg-primary-green before:opacity-0",
          isActive && "after:opacity-100 bg-current"
        )}
      ></span>
      <span>{title}</span>
    </button>
