...
export default function NewEmailDialog({
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const response = await instance.post(`${API_URL}/ai/api/v1/generate-email-template`, {
        user_prompt: prompt,
        model: "gpt-4o",
      });
    ...
    <Dialog defaultOpen={!generateHtml}>
      ...
      <DialogContent className="max-w-[986px]">
        <main className="flex-1 h-full w-full flex flex-col justify-center items-center gap-6 mb-24">
          <Image src="/logo/growstack.png" alt="" width={200} height={200} className="max-h-10 overflow-hidden" />
          <h1 className="text-3xl font-semibold text-center">Create email with AI</h1>
          <p className="text-center max-w-4xl mx-auto leading-relaxed text-primary-black text-opacity-70">
            Lorem ipsum dolor sit amet consectetur. A pharetra vitae at scelerisque eu et aliquam id. Sollicitudin suspendisse at elementum lorem. In quis quis
            et.
          </p>
          <div className="!bg-white shadow-box pl-4 pr-2 py-2 !rounded-2xl border border-[#E8E8E8] w-full max-w-3xl flex gap-4 shadow-xl shadow-gray-200/60">
            ...
            <input className="h-11 w-full" placeholder="An email to welcome new subscribers" value={prompt} onChange={handleInputChange} />
            <DialogClose>
              <button className="bg-primary-green p-3 rounded-2xl grid place-content-center" onClick={handleSubmit}>
                <SendIcon />
              </button>
            </DialogClose>
          </div>
