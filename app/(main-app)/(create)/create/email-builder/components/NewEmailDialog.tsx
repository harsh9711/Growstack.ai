...
export default function NewEmailDialog({
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.post(`${API_URL}/ai/api/v1/generate-email-template`, {
        user_prompt: prompt,
        model: "gpt-4o",
      });
    ...
    <Dialog defaultOpen={!generateHtml}>
      ...
      <DialogContent className="max-w-[986px]">
        <main className="flex-1 h-full w-full flex flex-col justify-center items-center gap-6 mb-24">
          <Image src="/logo/growstack.svg" alt="" width={200} height={200} className="max-h-10 overflow-hidden" />
