...
const Sidebar = ({ selectedTab, setSelectedTab }: { selectedTab: string; setSelectedTab: (value: string) => void }) => {
  const renderContent = () => {
    switch (selectedTab) {
      case "Widgets":
        return <Widgets />;
...
const Widgets = () => {
  return (
    <Motion transition={{ duration: 0.5 }} variants={{ hidden: { opacity: 0 }, visible: { opacity: 1 } }}>
      <div className="bg-primary-green px-4 py-2.5 text-white text-sm">General</div>
      <div className="p-4">
        <div className="grid grid-cols-3 gap-4">
          {Array(20)
            .fill(null)
            .map((_, index) => (
              <div key={index} className="bg-white border p-4 rounded-lg flex flex-col justify-center items-center gap-2">
                <Image src="/assets/widgets/rss.svg" alt="" width={50} height={50} />
