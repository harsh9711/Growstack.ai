...
export default function DocumentsTable() {
  ...
  const columns: ColumnDef<Document>[] = [
    {
      accessorKey: "doc_name",
      header: () => <div className="uppercase">Document Name</div>,
      cell: ({ row }) => <div className="capitalize">{row.getValue("doc_name")}</div>,
    },
    // {
    //   accessorKey: "workbook",
    //   header: () => <div className="uppercase">Workbook</div>,
  ...
  const handleGetDocumentsData = async () => {
    try {
      const response = await instance.get(`${API_URL}/users/api/v1/docs?page=${pagination.pageIndex + 1}&limit=10`);
  ...
  const handleDeleteDocs = (id: string) => {
    swal({
      ...
      if (willDelete) {
        try {
          const response = await instance.delete(`${API_URL}/users/api/v1/docs/${id}`);
