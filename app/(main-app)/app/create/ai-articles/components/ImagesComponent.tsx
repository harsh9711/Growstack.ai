      ...
      <div className="flex gap-5 items-start !mt-10">
          ...
          <div className="flex gap-3 !mt-2">
            <button
              ...
              className={clsx(
                ...
                isImageGenerationPending || (isArticlePending && "opacity-70 cursor-not-allowed")
              )}>
              {isImageGenerationPending ? <Spinner /> : images.length > 0 ? "Regenerate Image" : "Generate Image"}
            </button>
            <button
              onClick={generateArticle}
              disabled={(isArticlePending && images.length < 1) || images.length >= 1 || isImageGenerationPending || isArticlePending}
              className={clsx(
                "w-full p-2 h-12 !mt-4 text-white bg-primary-green rounded-xl hover:bg-opacity-90 flex justify-center items-center",
                isImageGenerationPending || (isArticlePending && images.length < 1) || (isArticlePending && "opacity-70 cursor-not-allowed"),
        ...
        <div className="w-full">
          ...
          {isImageGenerationPending && images.length < 1 && (
            <div className="h-40 grid place-content-center">Please hang on as we generate the image for you...</div>
          )}
          {images.length > 0 && (
            <div>
              <ul className="mt-5">
