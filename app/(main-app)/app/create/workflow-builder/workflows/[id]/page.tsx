...
const Page = () => {
  ...
  const [workFlowData, setWorkFlowData] = useState<WorkFlowData>({ actions: [], input_configs: [], output_configs: [], name: "" });
  ...
  const handleResumeWorkflow = async (choice: boolean) => {
    try {
      const response = await axios.get(`${API_URL}/workflow/api/v1/wrun/resume/${workFlowResults.workflow_runner_id}?ctaAction=${choice}`);
      setWorkFlowResults(response.data.data);
    } catch (error) {
  ...
  return (
    <div>
        ...
        <div>
          <Link href="/app/create/workflow-builder">
            <button className="text-[#212833] hover:bg-primary-green/10 sheen flex gap-2 px-3.5 py-2 rounded-full font-medium items-center">
              ...
              <h2 className="text-lg font-semibold">Back / {workFlowData.name}</h2>
            </button>
          </Link>
        </div>
        <div className="flex flex-row gap-10 text-md font-medium justify-between">
          <h2 onClick={() => scrollToSection(buildRef, "build")} className={`cursor-pointer ${activeSection === "build" ? "border-b-4 border-green-800" : ""}`}>
            Build
          </h2>
          <h2 onClick={() => scrollToSection(runRef, "run")} className={`cursor-pointer ${activeSection === "run" ? "border-b-4 border-green-800" : ""}`}>
            Run
          </h2>
          <h2 onClick={() => scrollToSection(tableRef, "table")} className={`cursor-pointer ${activeSection === "table" ? "border-b-4 border-green-800" : ""}`}>
            Table
          </h2>
        </div>
      </div>
      <div
        ref={containerRef}
        className="flex flex-row items-start overflow-x-scroll snap-x snap-mandatory scroll-smooth "
      ...
      >
        <div ref={buildRef} className="flex-shrink-0 w-full h-full justify-center bg-white flex flex-col gap-6 pb-24">
          <div className=" px-[550px]  p-10 snap-center gap-10 flex flex-col items-center pt-20">
            ...
            <button
              className={clsx("relative w-full z-[20] max-w-[340px] p-3 border rounded-xl flex items-center gap-4 cursor-pointer !border-primary-green")}
              ...
              <div
                className={clsx("absolute right-0  z-[10] top-0 h-full flex items-center pr-3 transition-transform duration-300", {
            ...
            <div className="border shadow-md rounded-xl">
              ...
              <div className="flex flex-row p-4  justify-between">
                <div className="flex flex-row gap-4">
                  {" "}
                  <Image src="/history.png" alt="go" width={30} height={10} /> <h2 className="text-lg">Build history</h2>
                </div>
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path
                    d="M7.40359 6.32015L15.8936 3.49015C19.7036 2.22015 21.7736 4.30015 20.5136 8.11015L17.6836 16.6002C15.7836 22.3102 12.6636 22.3102 10.7636 16.6002L9.92359 14.0802L7.40359 13.2402C1.69359 11.3402 1.69359 8.23016 7.40359 6.32015Z"
                    stroke="#034737"
                    stroke-width="2"
        ...
        <div ref={tableRef} className="flex-shrink-0 w-full h-full min-w-[1000px] bg-white p-10 snap-center gap-10 flex flex-col">
          <div className="flex justify-between items-center mb-4">
            <div className="flex flex-row gap-4 items-center">
              <input type="file" onChange={(e) => importData(e.target.files![0])} className="hidden" id="import-csv" />
              <label
                htmlFor="import-csv"
                className="border bg-primary-blue hover:bg-primary-blue-dark text-white py-2 px-4 rounded-lg text-sm cursor-pointer transition duration-300 ease-in-out flex items-center">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path
