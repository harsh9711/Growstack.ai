...
const Page = () => {
  ...
  const [workFlowResults, setWorkFlowResults] = useState<WorkFlowResults>({ outputs: [], status: true, failed_step: -1, temp_outputs: [], workflow_runner_id: "" });
    ...
    try {
      ...
      const payload = {
        ...
        inputs: workFlowData.input_configs.map((input) => ({
          variable_name: input.variable_name, variable_value: input.default_value
        })),
        outputs: workFlowData.output_configs.map((output) => ({ variable_name: output.display_name, variable_value: output.value, variable_type: output.type }))
      }
        ...
        <div>
          <Link href="/app/create/workflow-builder">
            <button className="text-[#212833] hover:bg-primary-green/10 sheen flex gap-2 px-3.5 py-2 rounded-full font-medium items-center">
              ...
              <h2 className='text-xl font-medium'>Back {" "}/ {workFlowData.name}</h2>
            </button>
          </Link>
        </div>
      ...
      >
        ...
        <div ref={runRef} className="flex-shrink-0 w-full min-w-[1000px] bg-white p-10 snap-center gap-10 rounded-xl flex flex-col">
          ...
          <div className="flex flex-col gap-10">
            ...
            {
              !workFlowResults.paused && workFlowResults.outputs.map((output, idx) => (
                <>
            ...
            {
              workFlowResults.paused && workFlowResults.temp_outputs.map((output, idx) => (
                <>
                  {workFlowResults.status && (workFlowResults.failed_step <= idx + 1) &&
                    <OutputCard key={idx} index={idx} output={output} />
                  }
          ...
          <div className="flex justify-between items-center mt-4">
            ...
            <div className="flex gap-2 items-center">
              <button
                ...
                className={`bg-gray-200 hover:bg-gray-300 text-gray-600 py-1 px-3 rounded-md text-sm ${currentPage === 1 ? 'cursor-not-allowed' : 'cursor-pointer'
                  }`}
              >
                Previous
              </button>
              <button
                ...
                className={`bg-gray-200 hover:bg-gray-300 text-gray-600 py-1 px-3 rounded-md text-sm ${currentPage === totalPages ? 'cursor-not-allowed' : 'cursor-pointer'
                  }`}
              >
                Next
              </button>
