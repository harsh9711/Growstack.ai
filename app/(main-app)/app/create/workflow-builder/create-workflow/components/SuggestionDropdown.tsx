...
const SuggestionDropdown = ({ dropdownRef, isDropdownVisible, handleSubOptionClick, setSuggestionOptions, suggestionOptions }: SuggestionDropdownProps) => {
    ...
    const toggleSuggestion = (index: number) => {
        setSuggestionOptions((prevState: SuggestionOption[]) => prevState.map((option, i) => {
            if (i === index) {
                return {
                    ...option,
    ...
    useEffect(() => {
        ...
        setSuggestionOptions((prevState: SuggestionOption[]) => prevState.map((option) => {
            return {
                ...option,
                            ...
                            <div key={index} className="mb-2">
                                ...
                                {suggestion.isExpanded && suggestion.subOptions.map((subOption, subIndex) => (
                                    subOption.show && (
                                        ...
                                        >
                                            <div className='flex flex-row gap-x-2'> {suggestion.icon}    <div>
                                                {subOption.label}</div>
                                            </div>
                                        </div>
                                    )
