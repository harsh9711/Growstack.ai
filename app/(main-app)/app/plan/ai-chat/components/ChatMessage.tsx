...
const ChatMessages: React.FC<ChatMessagesProps> = ({
  ...
  const extractFileUrl = (content: string) => {
    const fileUrlRegex = /!\[.*?\]\((https?:\/\/.*\.(?:doc|docx|txt|xls|xlsx|csv|pdf))\)/gi;
  ...
  const extractImageUrl = (content: string) => {
    const markdownImageRegex = /!\[.*?\]\((https?:\/\/.*\.(?:png|jpg|jpeg|gif))\)/gi;
  ...
  const removeUrlMarkdown = (content: string, image_tempurl: string | null, file_tempurl: string | null) => {
    if (image_tempurl || file_tempurl ) {
      const markdownImageRegex = /!\[.*?\]\(https?:\/\/.*\.(?:png|jpg|jpeg|gif|doc|docx|txt|xls|xlsx|csv|pdf)\)/gi;
    ...
    <>
      <div className="messages-container flex-1 w-full flex flex-col overflow-y-auto">
        {conversation.map((chat, index) => {
          ...
          chat.filename = file_tempurl ? getFileName(file_tempurl) : null || chat.filename;  
          const textContent = removeUrlMarkdown(chat.content, image_tempurl, file_tempurl);
          return (
