...
const BrandVoiceForm: React.FC<BrandVoiceFormProps> = ({ brandData, onClose, onUpdate }) => {
    const [brandName, setBrandName] = useState<string>(brandData.brand_name || '');
    const [websites] = useState<string[]>(brandData.websites || ['']);
    const [brandVoice, setBrandVoice] = useState<string>(brandData.brand_voice || '');
    const [description, setDescription] = useState<string>(brandData.description || '');
    const [isDefault, setIsDefault] = useState<boolean>(brandData.is_default || false);
