    ...
    const showTableHandler = () => {
      ...
      for (let i = 0; i < 5; i++) {
        stars.push(
          <span
            key={i}
            style={{ color: i < rating ? "#FFD700" : "gray", fontSize: "20px" }}
          >
            ★
          </span>
        );
                    ...
                    {places.map((place, index) => (
                      <TableRow key={index}>
                        ...
                        <TableCell>
                          ...
                          <div className="flex flex-row gap-4 text-md"><h2 className=" font-semibold">{place?.rating || "-"} </h2>  {"  "} {"  "} {"  "} {"  "}<h2 className="text-sky-500">{place?.ratingCount || "-"}</h2></div>
                        </TableCell>
                        <TableCell>
