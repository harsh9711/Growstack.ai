...
const outputType = [{ icon: <Trash2Icon size={20} />, label: "Delete Chat", value: "delete_chat" }];

const ChatArea = ({ selectedModel, addChatArea, onModelChange, conversation, handleChange, userPrompt, handleDelete }: ChatAreaProps) => {
    ...
    <div className="flex-1 flex flex-col !bg-white border border-[#E8E8E8] normal-box p-7 w-full justify-between min-w-[400px]">
      <div className="flex items-start justify-between mb-[10px]">
        ...
        <div className="flex items-center gap-2">
          ...
          <button type="button" onClick={addChatArea} className="p-1 hover:bg-gray-100 rounded-lg">
            <Plus size={20} />
          </button>
              ...
              <SelectContent>
                <SelectGroup>
                  {outputType.map(({ label, value, icon }) => (
                    <SelectItem value={value} key={value} className="pl-2 cursor-pointer">
...
const AIModel = ({ selectedOption, setSelectedOption, selectedOptionLabel }: AIModelProps) => {
    ...
    <Select value={selectedOption} onValueChange={setSelectedOption}>
      <SelectTrigger className="w-[70%] h-[34px] bg-white text-header border border-gray-300 rounded-xl flex items-center justify-between px-4">
        <SelectValue placeholder="Select an option">
          {selectedOptionLabel && (
            <div className="flex items-center gap-2">
              <span className="min-w-fit">{allModels.find((model) => model.value === selectedOption)?.icon}</span>
              {selectedOptionLabel}
            </div>
          )}
        </SelectValue>
...
const InitialMsg = ({ selectedOption, selectedOptionLabel }: InitialMsgProps) => {
  const selectedOptionModel = modelData.flatMap((provider) => provider.models).find((model) => model.value === selectedOption);
  return (
