...
const MessageList = ({
  ...
  onSelectMessage: (message: SidebarItemProps) => void;
}) => {
    ...
    const fetchMessages = async () => {
      try {
        const response = await instance.get(
          "/users/api/v1/social-media/profile/messages"
        );
        ...
        if (err.response) {
          if (
            ...
            "Please connect your social media account"
          ) {
            Swal.fire({
      ...
      {dataArray.map((dataItem, index) => (
        <>
          <SidebarItem
            ...
            message={
              dataItem.messages.messages[0]?.message || "No message available"
            }
            imageUrl={
            ...
            onClick={() =>
              onSelectMessage({
                ...
                time: formatRelativeDate(
                  dataItem.messages.messages[0]?.created
                ),
                author:
                  dataItem.messages.messages[0]?.senderDetails.username || "",
