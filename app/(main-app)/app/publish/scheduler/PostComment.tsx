...
import { CrossMark, GenAi, ImgVector } from "@/components/svgs";
...
const PostComment: FC<PostCommentProps> = ({
  ...
  const [fileInfo, setFileInfo] = useState<{ name: string; file: File } | null>(
    null
  );
    ...
    <>
      <Dialog open={openPostModel} onOpenChange={isGenPost}>
        ...
        >
                ...
                {showActions && (
                  <>
                    <div className="absolute mb-2 right-2 flex items-center space-x-2">
                      <button
                        className="text-[#034737] bg-transparent border-none"
                        onClick={reGenerate}
                      >
                        Retry
                      </button>
          ...
          {fileInfo && (
            <div
              className="flex items-center justify-between h-9 mt-2 m-2 bg-gray-100 border border-gray-300 rounded-lg"
            ...
            >
              ...
              <button className="text-red-500 ml-2" onClick={handleRemoveFile}>
                <IoMdClose size={20} />
              </button>
            </div>
          )}
