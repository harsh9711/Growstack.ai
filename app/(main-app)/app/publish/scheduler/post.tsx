  ...
  const fetchPostDetails = async () => {
    ...
    try {
      if (selectedIcon !== null) {
        ...
        const post = response.data.data.posts.filter(
          (postItems: any) => postItems.post !== ""
        );
        setPostDetails(post);
      }
      setLoading(false);
  ...
  const platformDetails = () => {
    ...
    if (profile !== undefined && profile && selectedIcon) {
      platformDetails = profile.activeSocialAccounts
        .filter(
          (account: any) => account.platform === selectedIcon.toLowerCase()
        )
        .map((account: any) => {
          return {
            ...
            <div className="flex flex-row gap-3 mt-2 px-2 w-full">
              <div className="flex flex-col gap-3">
                <div className="w-[50px] h-[50px] rounded-full shadow-lg bg-white flex items-center justify-center cursor-pointer">
                  ...
                  {selectedIcon.toLowerCase() === "linkedin" && (
                    <LinkedinIcon />
                  )}
                  ...
                  <div className="w-15">
                    <div className="w-[50px] h-[50px] rounded-full overflow-hidden flex items-center justify-center">
                      {userPostDetails.length > 0 &&
                        userPostDetails[0].userImage?.startsWith("http") && (
                          <Image
                            src={userPostDetails[0].userImage}
                            alt="User Avatar"
                            width={50}
                            height={50}
                          />
                  ...
                  <div className="flex flex-col gap-3 mt-2">
                    <div className="flex flex-row justify-between w-full h-[20px]">
                      <div className="flex flex-row items-center justify-start gap-1">
                        <p className="font-bold font-sans text-[16.7px] tracking-[-0.3px]">
                          {userPostDetails.length > 0 &&
                          userPostDetails[0].username !== undefined
                            ? userPostDetails[0].username
                              ? userPostDetails[0].username
                              : userPostDetails[0].displayName
                            : userPostDetails[0].displayName}
                        </p>
                      ...
                      <div className="cursor-pointer relative">
                        {visibleMenuIndex === index && (
                          <div
                            className="absolute right-0 mt-2 w-[100px] text-center items-center h-8 bg-white shadow-lg rounded-md z-10"
                            onClick={() => handleDeleteComment(post)}
                          >
                            <div className="mt-1 text-red-500">Delete Post</div>
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <p className="mb-3  text-left text-[16.7px] tracking-[-0.3px] text-[rgba(20,23,26,1)]">
                        {post.post
                          .split(" ")
                          .map((word: any, index: Key | null | undefined) =>
                            word.startsWith("#") ? (
                          ...
                          {post.media.map(
                            (
                              item: {
                                ...
                                mediaUrls: { mediaUrl: string | undefined }[];
                              },
                      ...
                      <div className="text-left mt-3">
                        {selectedIcon.toLowerCase() === "twitter" ? (
                          ...
                          >
                            ...
                            >
                              <path
                                d="M7 11V19C7 19.5523 6.55228 20 6 20H4C3.44772 20 3 19.5523 3 19V12C3 11.4477 3.44772 11 4 11H7C9.20914 11 11 9.20914 11 7V6C11 4.89543 11.8954 4 13 4C14.1046 4 15 4.89543 15 6V11H18C19.1046 11 20 11.8954 20 13L19 18C18.7046 19.2603 17.8892 20.0756 17 20H10C8.34315 20 7 18.6569 7 17"
                                stroke="#5B5D60"
                                stroke-width="1.3"
