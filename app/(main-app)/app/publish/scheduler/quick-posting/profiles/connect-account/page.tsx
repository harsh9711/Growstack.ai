...
export default function ConnectAccount() {
  ...
  // Dummy pages data
  const dummyPages = [
    { name: "DeadShot08 Gaming", imageUrl: "/dummy/dummy-page1.png", username: "DeadShot_gaming" },
    { name: "aamazon.in_", imageUrl: "/dummy/dummy-page2.png", username: "aamazon.in_" },
  ];
  ...
  return (
    <div className="w-full max-w-3xl bg-white p-10 rounded-3xl shadow-2xl shadow-gray-400 space-y-6">
      <h1 className="text-[28px] font-semibold">Choose {params.get("profile")} page</h1>
      <div className="flex justify-end !mt-2">
        {/* Toggle select all / unselect all */}
        <p className="underline text-primary-green cursor-pointer font-medium" onClick={handleSelectAll}>
          {selectAllLabel}
        </p>
      </div>
          ...
          <div
            ...
            <div className="flex items-center gap-5">
              ...
              <div className="space-y-2">
                ...
                <p className="text-primary-black text-opacity-50">{page.username}</p>
              </div>
            </div>
            <Checkbox checked={checkedPages.includes(page.name)} onCheckedChange={() => handleCheckboxChange(page.name)} />
          </div>
        ))}
      </div>
      <p className="text-primary-black text-opacity-70">
        You will have <span className="font-semibold">4</span> profiles remaining in your current plan.
      </p>
      <div className="flex justify-end gap-4">
        <Link href="/app/publish/scheduler/quick-posting/profiles">
          <button className="h-12 py-3.5 px-10 bg-white border border-primary-green text-primary-green rounded-xl mt-6">Back</button>
        </Link>
        <Link href={{ pathname: "/app/publish/scheduler/quick-posting/profiles/connect-account/finish", query: { profile: params.get("profile") } }}>
          <button className="h-12 py-3.5 px-10 bg-primary-green sheen rounded-xl text-white mt-6">Connect</button>
        </Link>
      </div>
    </div>
  );
