...
const Box = () => {
  return (
    <div className="flex  flex-row w-full mx-auto items-center justify-center max-w-[1240px] h-full sm:p-4 overflow-hidden">
          ...
          <div key={sectionIndex} className="flex flex-col gap-y-10">
            ...
            <div className="flex flex-col gap-y-8">
              {section.cards.map((card, cardIndex) => (
                <div
                  key={cardIndex}
                  className={`border w-full h-auto sm:h-[160px] p-4 flex items-center justify-center ${
      ...
      <div className="bg-[#EDEDED] max-w-[240px]  h-full w-full sm:w-1/4 rounded-2xl flex flex-col items-center justify-center gap-y-24 sm:gap-y-6 p-2">
        <div className="max-w-[200px] w-full h-[50px] py-2 text-[14px] sm:text-[18px] bg-white rounded-[10px] text-center">
          Timeline
        </div>
        <div className="flex flex-col gap-y-64 sm:gap-y-4 overflow-auto h-full p-2">
          {data.timeline.map((item, index) => (
            <div
              key={index}
              className="rounded-full flex flex-col items-center justify-center w-24 sm:w-52 bg-white text-[12px] sm:text-[14px] h-24 sm:h-52"
            >
              <ClockIcon />
              <h2 className="font-extrabold sm:text-[16px] text-[10px] mt-2">{item.time}</h2>
          ...
          <div key={sectionIndex} className="flex flex-col gap-y-10">
            ...
            <div className="flex flex-col gap-y-8">
              {section.cards.map((card, cardIndex) => (
                <div
                  key={cardIndex}
                  className={`border w-full h-auto sm:h-[160px] p-4 sm:text-[20px] text-[16px] flex items-center justify-center ${
                    section.title === "Before Growstack"
                      ? "rounded-l-[20px]"
                      : "rounded-r-[20px]"
                  }`}
