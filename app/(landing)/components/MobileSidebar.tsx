    ...
    >
      <div className="pl-8 flex flex-col justify-between pt-6 pb-10 px-4 h-full">
        <div className="flex items-center justify-between w-full">
          ...
          <div className="w-10 grid place-content-center ml-6">
            ...
            >
              ...
              ></span>
              <span
                className={`w-4 relative h-[2px] bg-[#8E93A4] transition  rounded-full duration-100 opacity-100 ${isOpen && "!opacity-0 invisible"} `}
              ></span>
              <span
        ...
        <div className="flex flex-col gap-10">
          {navLinks.map((link, index) => (
            <Link href={link.href} key={index}>
              <div
                className={`flex flex-col gap-[10px] max-w-fit cursor-pointer relative ${pathname === link.href && "text-primary-green"}`}
              >
        ...
        <div className="flex gap-x-4 items-center w-full">
          <Link
            href="/auth/login"
            className="py-3 px-8 rounded-xl hover:bg-primary-green sheen flex justify-center items-center whitespace-nowrap"
          >
            SIgn in
          </Link>
          <Link
            href="/auth/register"
            className="sheen bg-primary-green text-white py-3.5 px-9 rounded-xl flex gap-2 items-center justify-center whitespace-nowrap"
          >
            Sign up
