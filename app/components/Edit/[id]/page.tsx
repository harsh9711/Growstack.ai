...
console.log(assistantData);
          ...
          reset({
            ...
            category: assistantData.category, // Populate category
            icon: assistantData.icon,         // Populate icon
          });
        } catch (error) {
  ...
  const handleUpdate: SubmitHandler<ValidationSchemaType> = async (data) => {
    ...
    try {
      const changedFields: Partial<Assistant> = {
        ...
        icon:data.icon,
        custom_prompt: data.custom_prompt, // Include custom_prompt
        userInputs,
      };

      await axios.put(`${API_URL}/ai/api/v1/chat-template/${id}`, changedFields);
      toast.success('Assistant updated successfully');
      router.back();  // Navigate to the previous page after successful update
    } catch (error) {
      ...
      <section className="bg-white border border-[#E4E4E4] rounded-3xl p-10 mt-5">
        <form onSubmit={handleSubmit(handleUpdate)}>
          ...
          <div className="space-y-2">
                ...
                {/* Replace Input with Select for category */}
                <Select
                  {...register('category')}
                  defaultValue={assistant.category} // Pre-select current category
                >
             ...
             <div className="space-y-2">
                ...
                />
                {errors.custom_prompt && <p>{errors.custom_prompt.message}</p>}
              </div>
         {/* <div className="space-y-2 !mt-8">
          ...
          <div key={index} className="flex gap-4 items-center">
            ...
            <div className="flex justify-end mt-8">
              <button
                type="submit"
                className="bg-primary-green text-white font-medium px-6 py-2 rounded-lg hover:bg-primary-green-dark transition-colors duration-200"
                disabled={isPending}
              >
